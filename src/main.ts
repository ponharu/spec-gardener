...
const FOOTER = "🤖 Generated by Spec Gardener";
...
const buildSpecBody = (spec: string, originalBody: string): string => {
...
const applyResult = async (
  ...
  const newBody = buildSpecBody(result.body, specContext.body);
...
export const main = async (): Promise<void> => {
  ...
  try {
    ...
    issueNumber = isPullRequestEvent
    ...
    const prompt = adapter.buildPrompt(specContext, customPrompt);
    ...
    await applyResult(octokit, owner, repo, issueNumber, result, specContext);
  } catch (error) {
    const message =
