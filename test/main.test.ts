...
describe("main", () => {
  ...
  it("places command hint in footer section for question comments", async () => {
    ...
    // Check the structure: content first, then separator, then hint, then footer
    const contentIndex = body.indexOf("@alice Need clarification");
    const separatorIndex = body.indexOf("---");
    const hintIndex = body.indexOf("💡 Type `/spec-gardener help` for available commands");
    const footerIndex = body.indexOf("🤖 Generated by Spec Gardener");
  ...
  it("places command hint in footer section for error comments", async () => {
    ...
    // Check the structure: error message first, then separator, then hint, then footer
    const errorIndex = body.indexOf("Spec Gardener encountered an error");
    const separatorIndex = body.indexOf("---");
    const hintIndex = body.indexOf("💡 Type `/spec-gardener help` for available commands");
    const footerIndex = body.indexOf("🤖 Generated by Spec Gardener");
    
    expect(errorIndex).toBeGreaterThan(-1);
    expect(separatorIndex).toBeGreaterThan(errorIndex);
    expect(hintIndex).toBeGreaterThan(separatorIndex);
    expect(footerIndex).toBeGreaterThan(hintIndex);
  });
  ...
  it("places command hint in footer section for complete result comments", async () => {
    ...
    // Check the structure: content first, then separator, then hint, then footer
    const contentIndex = body.indexOf("@alice Updated successfully");
    const separatorIndex = body.indexOf("---");
    const hintIndex = body.indexOf("💡 Type `/spec-gardener help` for available commands");
    const footerIndex = body.indexOf("🤖 Generated by Spec Gardener");
